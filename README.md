...
[![Tasks Solved](https://img.shields.io/badge/tasks_solved-186%2F400-blue)](networks/)
[![Local Score](https://img.shields.io/badge/local_score-2521.58-success)](networks/build_summary.json)
[![Public Score](https://img.shields.io/badge/public_score-2510.03-blue)](https://www.kaggle.com/competitions/neurogolf-2026)
...
| Metric · 指标 | Value · 数值 |
|---|---|
| **Tasks solved · 通过任务** | **186 / 400** |
| **Local score · 本地总分** | **2521.58** — clean-room scorer over `build_summary.json` · 独立评分器统计 |
| **Public score · 公开分数 (Kaggle)** | **2510.03** — leaderboard-confirmed at 185/400 (exact match to local); task 186 submitting · 已确认 185/400（与本地完全一致）；186 提交中 |
| Solvers · 求解器 | in 9 families · 分 9 类 |
| Unit tests · 单元测试 | 256 passing · 256 个全部通过 |
| Networks · 网络文件 | 186 × `networks/taskNNN.onnx` (one per solved task) · 每个解出任务一个 |

> Local score is **2521.58 / 186 tasks**; the Kaggle leaderboard is confirmed at **2510.03 / 185** (exact match to local), with task 186 in the current submission. The v1 → v46 progression lives in [Submission history](#history).
> 本地分数 **2521.58 / 186 解**；Kaggle 排行榜确认 **2510.03 / 185**（与本地完全一致），186 在本次提交中。v1 → v46 进展见 [提交历史](#history)。
...
| v118 | + stamp-at-markers (75); +bbox-recolor-ones opt (70) | 186 | _2521.58 local · submitting_ |
