...
[![Tasks Solved](https://img.shields.io/badge/tasks_solved-190%2F400-blue)](networks/)
[![Local Score](https://img.shields.io/badge/local_score-2576.33-success)](networks/build_summary.json)
[![Public Score](https://img.shields.io/badge/public_score-2563.36-blue)](https://www.kaggle.com/competitions/neurogolf-2026)
...
| Metric · 指标 | Value · 数值 |
|---|---|
| **Tasks solved · 通过任务** | **190 / 400** |
| **Local score · 本地总分** | **2576.33** — clean-room scorer over `build_summary.json` · 独立评分器统计 |
| **Public score · 公开分数 (Kaggle)** | **2563.36** — leaderboard-confirmed at 189/400 (exact match to local); task 190 submitting · 已确认 189/400（与本地完全一致）；190 提交中 |
| Solvers · 求解器 | in 9 families · 分 9 类 |
| Unit tests · 单元测试 | 264 passing · 264 个全部通过 |
| Networks · 网络文件 | 190 × `networks/taskNNN.onnx` (one per solved task) · 每个解出任务一个 |

> Local score is **2576.33 / 190 tasks**; the Kaggle leaderboard is confirmed at **2563.36 / 189** (exact match to local), with task 190 in the current submission. The v1 → v46 progression lives in [Submission history](#history).
> 本地分数 **2576.33 / 190 解**；Kaggle 排行榜确认 **2563.36 / 189**（与本地完全一致），190 在本次提交中。v1 → v46 进展见 [提交历史](#history)。
...
| v122 | + edge-frame (114) | 190 | _2576.33 local · submitting_ |
