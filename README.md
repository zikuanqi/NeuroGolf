...
[![Tasks Solved](https://img.shields.io/badge/tasks_solved-214%2F400-blue)](networks/)
[![Local Score](https://img.shields.io/badge/local_score-2883.25-success)](networks/build_summary.json)
[![Public Score](https://img.shields.io/badge/public_score-2830.46-blue)](https://www.kaggle.com/competitions/neurogolf-2026)
...
| Metric · 指标 | Value · 数值 |
|---|---|
| **Tasks solved · 通过任务** | **214 / 400** |
| **Local score · 本地总分** | **2883.25** — clean-room scorer over `build_summary.json` · 独立评分器统计 |
...
| Unit tests · 单元测试 | 314 passing · 314 个全部通过 |
| Networks · 网络文件 | 214 × `networks/taskNNN.onnx` (one per solved task) · 每个解出任务一个 |

> Local score is **2883.25 / 214 tasks**; the Kaggle leaderboard is confirmed at **2830.46 / 210** (exact match to local) — **past the halfway mark**, with tasks 211+ queued for the next daily submission. The v1 → v46 progression lives in [Submission history](#history).
> 本地分数 **2883.25 / 214 解**；Kaggle 排行榜确认 **2830.46 / 210**（与本地完全一致）—— **已过半**，211 及之后待每日额度提交。v1 → v46 进展见 [提交历史](#history)。
...
| v145 | + drop-one-recolor (261, new) + midpoint-fill-h (258 opt) | 214 | _2883.25 local · queued_ |
