...
[![Tasks Solved](https://img.shields.io/badge/tasks_solved-189%2F400-blue)](networks/)
[![Local Score](https://img.shields.io/badge/local_score-2563.37-success)](networks/build_summary.json)
[![Public Score](https://img.shields.io/badge/public_score-2521.58-blue)](https://www.kaggle.com/competitions/neurogolf-2026)
...
| Metric · 指标 | Value · 数值 |
|---|---|
| **Tasks solved · 通过任务** | **189 / 400** |
| **Local score · 本地总分** | **2563.37** — clean-room scorer over `build_summary.json` · 独立评分器统计 |
| **Public score · 公开分数 (Kaggle)** | **2521.58** — leaderboard-confirmed at 186/400 (exact match to local); tasks 187–189 queued for next daily reset · 已确认 186/400（与本地完全一致）；187–189 待额度重置后提交 |
| Solvers · 求解器 | in 9 families · 分 9 类 |
| Unit tests · 单元测试 | 262 passing · 262 个全部通过 |
| Networks · 网络文件 | 189 × `networks/taskNNN.onnx` (one per solved task) · 每个解出任务一个 |

> Local score is **2563.37 / 189 tasks**; the Kaggle leaderboard is confirmed at **2521.58 / 186** (exact match to local), with tasks 187–189 queued for the next daily reset (5/5 submissions used today). The v1 → v46 progression lives in [Submission history](#history).
> 本地分数 **2563.37 / 189 解**；Kaggle 排行榜确认 **2521.58 / 186**（与本地完全一致），187–189 待下次每日额度重置后提交（今日 5/5 已用）。v1 → v46 进展见 [提交历史](#history)。
...
| v121 | + stack-to-band (93) | 189 | _2563.37 local · queued_ |
