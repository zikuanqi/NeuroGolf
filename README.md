...
[![Tests](https://img.shields.io/badge/tests-310%20passing-brightgreen)](tests/)
[![Tasks Solved](https://img.shields.io/badge/tasks_solved-213%2F400-blue)](networks/)
[![Local Score](https://img.shields.io/badge/local_score-2868.98-success)](networks/build_summary.json)
...
| Metric · 指标 | Value · 数值 |
|---|---|
| **Tasks solved · 通过任务** | **213 / 400** |
| **Local score · 本地总分** | **2868.98** — clean-room scorer over `build_summary.json` · 独立评分器统计 |
...
| Unit tests · 单元测试 | 310 passing · 310 个全部通过 |
| Networks · 网络文件 | 213 × `networks/taskNNN.onnx` (one per solved task) · 每个解出任务一个 |

> Local score is **2868.98 / 213 tasks**; the Kaggle leaderboard is confirmed at **2704.10 / 200** (exact match to local) — **past the halfway mark**, with tasks 201+ queued for the next daily reset. The v1 → v46 progression lives in [Submission history](#history).
> 本地分数 **2868.98 / 213 解**；Kaggle 排行榜确认 **2704.10 / 200**（与本地完全一致）—— **已过半**，201 及之后待下次每日额度重置后提交。v1 → v46 进展见 [提交历史](#history)。
