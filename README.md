...
| **Public score · 公开分数 (Kaggle)** | **2484.64** — leaderboard-confirmed at 183/400 (exact match to local) · 已确认 183/400（与本地完全一致） |
...
> Local score is **2484.65 / 183 tasks**; the Kaggle leaderboard is confirmed at **2484.64 / 183** (exact match to local, to ~0.01). The v1 → v46 progression lives in [Submission history](#history).
> 本地分数 **2484.65 / 183 解**；Kaggle 排行榜确认 **2484.64 / 183**（与本地完全一致，误差约 0.01）。v1 → v46 进展见 [提交历史](#history)。
