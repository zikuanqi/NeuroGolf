...
[![Tasks Solved](https://img.shields.io/badge/tasks_solved-197%2F400-blue)](networks/)
[![Local Score](https://img.shields.io/badge/local_score-2665.69-success)](networks/build_summary.json)
...
| Metric · 指标 | Value · 数值 |
|---|---|
| **Tasks solved · 通过任务** | **197 / 400** |
| **Local score · 本地总分** | **2665.69** — clean-room scorer over `build_summary.json` · 独立评分器统计 |
| **Public score · 公开分数 (Kaggle)** | **2652.93** — leaderboard-confirmed at 196/400 (exact match to local); task 197 submitting · 已确认 196/400（与本地完全一致）；197 提交中 |
| Solvers · 求解器 | in 9 families · 分 9 类 |
| Unit tests · 单元测试 | 278 passing · 278 个全部通过 |
| Networks · 网络文件 | 197 × `networks/taskNNN.onnx` (one per solved task) · 每个解出任务一个 |

> Local score is **2665.69 / 197 tasks**; the Kaggle leaderboard is confirmed at **2652.93 / 196** (exact match to local), with task 197 in the current submission. The v1 → v46 progression lives in [Submission history](#history).
> 本地分数 **2665.69 / 197 解**；Kaggle 排行榜确认 **2652.93 / 196**（与本地完全一致），197 在本次提交中。v1 → v46 进展见 [提交历史](#history)。
...
| v129 | + edge-pair-lines (161) | 197 | _2665.69 local · submitting_ |
