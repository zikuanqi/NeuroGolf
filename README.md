...
[![Tasks Solved](https://img.shields.io/badge/tasks_solved-198%2F400-blue)](networks/)
[![Local Score](https://img.shields.io/badge/local_score-2679.33-success)](networks/build_summary.json)
...
| Metric · 指标 | Value · 数值 |
|---|---|
| **Tasks solved · 通过任务** | **198 / 400** |
| **Local score · 本地总分** | **2679.33** — clean-room scorer over `build_summary.json` · 独立评分器统计 |
| **Public score · 公开分数 (Kaggle)** | **2665.68** — leaderboard-confirmed at 197/400 (matches local to ~0.01); task 198 submitting · 已确认 197/400（与本地一致，误差约 0.01）；198 提交中 |
| Solvers · 求解器 | in 9 families · 分 9 类 |
| Unit tests · 单元测试 | 280 passing · 280 个全部通过 |
| Networks · 网络文件 | 198 × `networks/taskNNN.onnx` (one per solved task) · 每个解出任务一个 |

> Local score is **2679.33 / 198 tasks**; the Kaggle leaderboard is confirmed at **2665.68 / 197** (matches local to ~0.01), with task 198 in the current submission. The v1 → v46 progression lives in [Submission history](#history).
> 本地分数 **2679.33 / 198 解**；Kaggle 排行榜确认 **2665.68 / 197**（与本地一致，误差约 0.01），198 在本次提交中。v1 → v46 进展见 [提交历史](#history)。
