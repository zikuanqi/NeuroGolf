...
[![Tasks Solved](https://img.shields.io/badge/tasks_solved-183%2F400-blue)](networks/)
[![Local Score](https://img.shields.io/badge/local_score-2484.65-success)](networks/build_summary.json)
...
| Metric · 指标 | Value · 数值 |
|---|---|
| **Tasks solved · 通过任务** | **183 / 400** |
| **Local score · 本地总分** | **2484.65** — clean-room scorer over `build_summary.json` · 独立评分器统计 |
| **Public score · 公开分数 (Kaggle)** | **2458.66** — leaderboard-confirmed at 181/400; tasks 182–183 built/verified locally, submitting · 已确认 181/400；182–183 已本地验证，提交中 |
| Solvers · 求解器 | in 9 families · 分 9 类 |
| Unit tests · 单元测试 | 250 passing · 250 个全部通过 |
| Networks · 网络文件 | 183 × `networks/taskNNN.onnx` (one per solved task) · 每个解出任务一个 |

> Local score is **2484.65 / 183 tasks**; the Kaggle leaderboard is confirmed at **2458.66 / 181** (exact match to local), with tasks 182–183 in the current submission. The v1 → v46 progression lives in [Submission history](#history).
> 本地分数 **2484.65 / 183 解**；Kaggle 排行榜确认 **2458.66 / 181**（与本地完全一致），182–183 在本次提交中。v1 → v46 进展见 [提交历史](#history)。
...
| v114 | + align-to-anchor (30) | 182 | _2471.39 local · submitting_ |
| v115 | + panel-complete (33) | 183 | _2484.65 local · submitting_ |
