...
[![Tasks Solved](https://img.shields.io/badge/tasks_solved-180%2F400-blue)](networks/)
[![Local Score](https://img.shields.io/badge/local_score-2445.96-success)](networks/build_summary.json)
...
| Metric · 指标 | Value · 数值 |
|---|---|
| **Tasks solved · 通过任务** | **180 / 400** |
| **Local score · 本地总分** | **2445.96** — clean-room scorer over `build_summary.json` · 独立评分器统计 |
| **Public score · 公开分数 (Kaggle)** | **2389.93** — leaderboard-confirmed at 175/400; tasks 176–180 built/verified locally, queued for the next daily reset · 已确认 175/400；176–180 已本地验证，待额度重置后提交 |
| Solvers · 求解器 | 157, in 9 families · 共 157 个，分 9 类 |
| Unit tests · 单元测试 | 244 passing · 244 个全部通过 |
| Networks · 网络文件 | 157 × `networks/taskNNN.onnx` (one per solved task) · 每个解出任务一个 |

> Local score is **2445.96 / 180 tasks**; the Kaggle leaderboard is confirmed at **2389.93 / 175** (exact match to local), with tasks 176–180 awaiting the next submission window. The v1 → v46 progression lives in [Submission history](#history).
> 本地分数 **2445.96 / 180 解**；Kaggle 排行榜确认 **2389.93 / 175**（与本地完全一致），176–180 待下次提交窗口。v1 → v46 进展见 [提交历史](#history)。
