...
[![Tasks Solved](https://img.shields.io/badge/tasks_solved-155%2F400-blue)](networks/)
[![Local Score](https://img.shields.io/badge/local_score-2140.73-success)](networks/build_summary.json)
...
| Metric · 指标 | Value · 数值 |
|---|---|
| **Tasks solved · 通过任务** | **155 / 400** |
| **Local score · 本地总分** | **2140.73** — clean-room scorer over `build_summary.json` · 独立评分器统计 |
| **Public score · 公开分数 (Kaggle)** | **2128.66** — leaderboard-confirmed at 154/400; task 155 (diagonal-markers) is built/verified locally, submission queued for the next daily reset · 已确认 154/400；第 155 个已本地验证，待额度重置后提交 |
| Solvers · 求解器 | 132, in 9 families · 共 132 个，分 9 类 |
| Unit tests · 单元测试 | 194 passing · 194 个全部通过 |
| Networks · 网络文件 | 155 × `networks/taskNNN.onnx` (one per solved task) · 每个解出任务一个 |

> Local score is **2140.73 / 155 tasks**; the Kaggle leaderboard is confirmed at **2128.66 / 154** (matching local to ~0.01), with task 155 awaiting the next submission window. The v1 → v46 progression lives in [Submission history](#history).
> 本地分数 **2140.73 / 155 解**；Kaggle 排行榜确认 **2128.66 / 154**（与本地一致，误差 ~0.01），第 155 个待下次提交窗口。v1 → v46 进展见 [提交历史](#history)。
...
ARC task (JSON)
 ...
 │    for solver in ALL_SOLVERS:          ~132 pattern‑specific solvers   │
...
132 solvers in 9 families. Each is verified against the official scorer; `Params` is the parameter count of a representative network (it can vary slightly per task because detected constants differ). A few solvers are registered as capabilities but currently win no task (dominated or non-matching on the present set); these are marked `—†`.

132 个求解器分为 9 类。`Params` 列为代表性网络的参数量（不同任务因检测出的常量不同会略有差异）。少数求解器已登记但当前未中标（被更省的求解器击败或不匹配现有任务），以 `—†` 标记。
...
│   └── solvers/          # 132 pattern-specific solvers    · 各类求解器
...
```bash
python -m pytest tests/ -q        # 194 passing
```
