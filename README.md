...
[![Tests](https://img.shields.io/badge/tests-266%20passing-brightgreen)](tests/)
[![Tasks Solved](https://img.shields.io/badge/tasks_solved-191%2F400-blue)](networks/)
[![Local Score](https://img.shields.io/badge/local_score-2589.36-success)](networks/build_summary.json)
[![Public Score](https://img.shields.io/badge/public_score-2576.32-blue)](https://www.kaggle.com/competitions/neurogolf-2026)
...
| Metric · 指标 | Value · 数值 |
|---|---|
| **Tasks solved · 通过任务** | **191 / 400** |
| **Local score · 本地总分** | **2589.36** — clean-room scorer over `build_summary.json` · 独立评分器统计 |
| **Public score · 公开分数 (Kaggle)** | **2576.32** — leaderboard-confirmed at 190/400 (exact match to local); task 191 submitting · 已确认 190/400（与本地完全一致）；191 提交中 |
| Solvers · 求解器 | in 9 families · 分 9 类 |
| Unit tests · 单元测试 | 266 passing · 266 个全部通过 |
| Networks · 网络文件 | 191 × `networks/taskNNN.onnx` (one per solved task) · 每个解出任务一个 |

> Local score is **2589.36 / 191 tasks**; the Kaggle leaderboard is confirmed at **2576.32 / 190** (exact match to local), with task 191 in the current submission. The v1 → v46 progression lives in [Submission history](#history).
> 本地分数 **2589.36 / 191 解**；Kaggle 排行榜确认 **2576.32 / 190**（与本地完全一致），191 在本次提交中。v1 → v46 进展见 [提交历史](#history)。
...
| v123 | + eight-center-crop (121) | 191 | _2589.36 local · submitting_ |
