...
> Local score is **2602.00 / 192 tasks**; the Kaggle leaderboard is confirmed at **2589.36 / 191** (exact match to local), with task 192 in the current submission. The v1 → v46 progression lives in [Submission history](#history).
> 本地分数 **2602.00 / 192 解**；Kaggle 排行榜确认 **2589.36 / 191**（与本地完全一致），192 在本次提交中。v1 → v46 进展见 [提交历史](#history)。
