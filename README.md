...
[![Tasks Solved](https://img.shields.io/badge/tasks_solved-107%2F400-blue)](networks/)
[![Local Score](https://img.shields.io/badge/local_score-1520.83-success)](networks/build_summary.json)
...
| Metric · 指标 | Value · 数值 |
|---|---|
| **Tasks solved · 通过任务** | **107 / 400** |
| **Local score · 本地总分** | **1520.83** — clean-room scorer over `build_summary.json` · 独立评分器统计 |
| **Public score · 公开分数 (Kaggle)** | **1520.83** — leaderboard-confirmed (107/400); local = official to 2 decimals · 排行榜确认 |
| Solvers · 求解器 | 86, in 9 families · 共 86 个，分 9 类 |
| Unit tests · 单元测试 | 108 passing · 108 个全部通过 |
| Networks · 网络文件 | 107 × `networks/taskNNN.onnx` (one per solved task) · 每个解出任务一个 |

> Latest Kaggle submission **1520.83 / 107 tasks** (2026-06-03) — the clean-room local scorer matches the official public score to two decimals. The v1 → v46 progression lives in [Submission history](#history).
> 最新 Kaggle 提交 **1520.83 / 107 解**（2026-06-03）—— 本地独立评分器与官方分数小数点后两位一致。v1 → v46 进展见 [提交历史](#history)。
...
ARC task (JSON)
 ...
 │    for solver in ALL_SOLVERS:          ~86 pattern‑specific solvers    │
...
86 solvers in 9 families. Each is verified against the official scorer; `Params` is the parameter count of a representative network (it can vary slightly per task because detected constants differ). A few solvers are registered as capabilities but currently win no task (dominated or non-matching on the present set); these are marked `—†`.

86 个求解器分为 9 类。`Params` 列为代表性网络的参数量（不同任务因检测出的常量不同会略有差异）。少数求解器已登记但当前未中标（被更省的求解器击败或不匹配现有任务），以 `—†` 标记。
...
│   └── solvers/          # 86 pattern-specific solvers     · 各类求解器
...
```bash
python -m pytest tests/ -q        # 108 passing
```
