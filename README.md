...
[![Tasks Solved](https://img.shields.io/badge/tasks_solved-207%2F400-blue)](networks/)
[![Local Score](https://img.shields.io/badge/local_score-2790.45-success)](networks/build_summary.json)
...
| Metric · 指标 | Value · 数值 |
|---|---|
| **Tasks solved · 通过任务** | **207 / 400** |
| **Local score · 本地总分** | **2790.45** — clean-room scorer over `build_summary.json` · 独立评分器统计 |
| **Public score · 公开分数 (Kaggle)** | **2704.10** — leaderboard-confirmed at 200/400 (exact match to local); tasks 201–207 queued for next daily reset · 已确认 200/400（与本地完全一致）；201–207 待额度重置后提交 |
| Solvers · 求解器 | in 9 families · 分 9 类 |
| Unit tests · 单元测试 | 298 passing · 298 个全部通过 |
| Networks · 网络文件 | 207 × `networks/taskNNN.onnx` (one per solved task) · 每个解出任务一个 |

> Local score is **2790.45 / 207 tasks**; the Kaggle leaderboard is confirmed at **2704.10 / 200** (exact match to local) — **past the halfway mark**, with tasks 201–207 queued for the next daily reset (5/5 submissions used today). The v1 → v46 progression lives in [Submission history](#history).
> 本地分数 **2790.45 / 207 解**；Kaggle 排行榜确认 **2704.10 / 200**（与本地完全一致）—— **已过半**，201–207 待下次每日额度重置后提交（今日 5/5 已用）。v1 → v46 进展见 [提交历史](#history)。
