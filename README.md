...
[![Tasks Solved](https://img.shields.io/badge/tasks_solved-177%2F400-blue)](networks/)
[![Local Score](https://img.shields.io/badge/local_score-2409.59-success)](networks/build_summary.json)
...
| Metric · 指标 | Value · 数值 |
|---|---|
| **Tasks solved · 通过任务** | **177 / 400** |
| **Local score · 本地总分** | **2409.59** — clean-room scorer over `build_summary.json` · 独立评分器统计 |
| **Public score · 公开分数 (Kaggle)** | **2389.93** — leaderboard-confirmed at 175/400; tasks 176–177 (bbox-fill, cross-center) built/verified locally, queued for the next daily reset · 已确认 175/400；176–177 已本地验证，待额度重置后提交 |
| Solvers · 求解器 | 154, in 9 families · 共 154 个，分 9 类 |
| Unit tests · 单元测试 | 238 passing · 238 个全部通过 |
| Networks · 网络文件 | 157 × `networks/taskNNN.onnx` (one per solved task) · 每个解出任务一个 |

> Local score is **2409.59 / 177 tasks**; the Kaggle leaderboard is confirmed at **2389.93 / 175** (exact match to local), with tasks 176–177 awaiting the next submission window. The v1 → v46 progression lives in [Submission history](#history).
> 本地分数 **2409.59 / 177 解**；Kaggle 排行榜确认 **2389.93 / 175**（与本地完全一致），176–177 待下次提交窗口。v1 → v46 进展见 [提交历史](#history)。
...
ARC task (JSON)
 ...
 │    for solver in ALL_SOLVERS:          ~154 pattern‑specific solvers   │
...
154 solvers in 9 families. Each is verified against the official scorer; `Params` is the parameter count of a representative network (it can vary slightly per task because detected constants differ). A few solvers are registered as capabilities but currently win no task (dominated or non-matching on the present set); these are marked `—†`.

154 个求解器分为 9 类。`Params` 列为代表性网络的参数量（不同任务因检测出的常量不同会略有差异）。少数求解器已登记但当前未中标（被更省的求解器击败或不匹配现有任务），以 `—†` 标记。
...
│   └── solvers/          # 154 pattern-specific solvers    · 各类求解器
